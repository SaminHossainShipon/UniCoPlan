...
// ---- login gate: Google Sign-In only ----
// Replace with your own OAuth 2.0 Web Client ID from
// https://console.cloud.google.com/apis/credentials — create an "OAuth client ID"
// of type "Web application" and add this site's exact URL(s) under "Authorized
// JavaScript origins". A client ID is not a secret; it's fine for it to live here.
const GOOGLE_CLIENT_ID = 'YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com';

// Optional: restrict sign-in to a specific Google Workspace domain (e.g. your
// university's), so any Gmail account can't get in — only accounts on that domain.
// Leave as null to allow any Google account.
const ALLOWED_HOSTED_DOMAIN = null; // e.g. 'university.edu'

const AUTH_KEY = 'unicoplan:auth:' + ROLE;

// IMPORTANT: this app has no backend. The block below reads the signed-in user's
// name/email/picture out of the Google ID token so the UI can show who's logged
// in — it does NOT cryptographically verify the token's signature. That's fine for
// gating the UI (which is all this app currently does), but it means a
// sufficiently technical visitor could still forge a fake token client-side.
// Before this app stores or serves any real student/faculty data, add a server
// that verifies the ID token (Google's tokeninfo endpoint or a server-side auth
// library) and issues its own session — never trust the client-decoded payload
// for anything that guards real data.
function isLoggedIn(){
...
function setLoggedInUser(profile){
  try { sessionStorage.setItem(AUTH_KEY, JSON.stringify(profile)); }
  catch (e) { /* ignore */ }
}
...
// Decodes (does not verify) a Google ID token's payload — see the note above.
...
function handleGoogleCredential(response){
  let profile;
  try { profile = decodeGoogleCredential(response.credential); }
  catch (e){ return; }

  if (ALLOWED_HOSTED_DOMAIN && profile.hd !== ALLOWED_HOSTED_DOMAIN){
    const flash = document.getElementById('loginFlash');
    if (flash) showFlash(flash, 'error', `Please sign in with a @${ALLOWED_HOSTED_DOMAIN} account.`);
    clearLoggedInUser();
    return;
  }

  setLoggedInUser({ name: profile.name, email: profile.email, picture: profile.picture });
  renderApp();
}
...
function renderGoogleButton(){
  if (typeof google === 'undefined' || !google.accounts || !google.accounts.id){
    setTimeout(renderGoogleButton, 150); // GIS script still loading
    return;
  }
...
function renderHeader(){
  ...
  header.classList[loggedIn ? 'add' : 'remove']('header-has-logout');
  header.innerHTML = `
    ...
    ${loggedIn ? `
      <div class="header-user">
        ${user && user.picture ? `<img class="header-avatar" src="${user.picture}" alt="" referrerpolicy="no-referrer">` : ''}
        <span class="header-username">${user ? escapeHtml(user.name || user.email || '') : ''}</span>
...
// Login screen shown in place of the panel's normal content until the person
// signs in for this browser session (sessionStorage — closing the tab signs
// them back out; a plain refresh does not).
  ...
  main.innerHTML = `
    <div class="login-wrap">
      <div class="form-card login-card">
        ...
        <p class="muted login-sub">Sign in with Google to continue to the ${roleLabel.toLowerCase()} panel.</p>
...
function applyCombination(comboId){
  ...
  current.forEach(id => {
    ...
    if (c) c.seatsTaken = Math.max(0, c.seatsTaken - 1);
  });
  ...
  available.slice(0, MAX_PLAN_COURSES).forEach(id => {
    const c = sem.courses.find(cc => cc.id === id);
    if (c && c.seatsTaken < c.capacity){ c.seatsTaken++; applied.push(id); }
  });
...
function removeCourse(sem, courseId){
  const course = sem.courses.find(c => c.id === courseId);
  if (course) course.seatsTaken = Math.max(0, course.seatsTaken - 1);
