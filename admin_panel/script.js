...
async function onAuthorized(){
  ...
  await loadProfile();
  await loadMyAdminRoleLabel();
}

// Pulls the signed-in admin's own admin_role label from user_roles, for the
// profile-menu "Role" line (step 2's Role dropdown). Only meaningful for
// appRole === 'admin' — the hardcoded main admin's role is always "Main
// Admin" and students don't have a user_roles row at all. A non-main admin
// can read only *their own* row here — see the user_roles_select_self
// policy in supabase/migrations/002_admin_roles_and_comments.sql.
async function loadMyAdminRoleLabel(){
  if (!sb || !authSession || appRole !== 'admin'){ state.myAdminRoleLabel = null; return; }
  try {
    const email = (authSession.user.email || '').toLowerCase();
    const { data, error } = await sb.from('user_roles').select('admin_role').eq('email', email).maybeSingle();
    if (error) throw error;
    state.myAdminRoleLabel = adminRoleLabel(data && data.admin_role);
  } catch (e){
    console.error('Could not load your admin role.', e);
    state.myAdminRoleLabel = 'Admin';
  }
}

// Role shown in the profile-menu dropdown — reflects the admin sub-type
// (Main Admin / this account's chosen admin_role) or plain "Student".
function getRoleLabel(){
  if (appRole === 'main_admin') return 'Main Admin';
  if (appRole === 'admin') return state.myAdminRoleLabel || 'Admin';
...
const state = {
  ...
  profileLoaded: false,
  myAdminRoleLabel: null, // this admin's own admin_role label (from user_roles), for the profile-menu "Role" line
...
// Admin sub-types shown in "Manage access" (as a Role dropdown) and in a
// signed-in admin's own profile-menu "Role" line. Extendable — add more
// { value, label } entries here as new admin sub-roles are needed; `value`
// is what's stored in user_roles.admin_role, `label` is display-only.
const ADMIN_ROLES = [
...
function adminRoleLabel(value){
  const found = ADMIN_ROLES.find(r => r.value === value);
  return found ? found.label : (value || 'Admin');
}
...
function adminAccessTemplate(){
  ...
  const roleOptions = ADMIN_ROLES.map(r => `<option value="${r.value}" ${r.value === 'semester_adjust' ? 'selected' : ''}>${escapeHtml(r.label)}</option>`).join('');
  return `
    <section class="form-card" style="max-width:620px">
      ...
      <form id="addAccessForm" class="inline-form">
        ...
        <select id="newAdminRole" title="Role">${roleOptions}</select>
        <button type="submit" class="btn btn-ghost">+ Grant admin access</button>
      </form>
      <p class="empty-note" style="padding:0 0 4px">"Main Admin" here only labels the account — the account with full, irreversible main-admin access is fixed in the database and can't be changed from this page.</p>
    ...
    <div class="course-row" data-access-email="${escapeHtml(r.email)}">
      <div class="course-row-main">
        <div>
          ...
          <div class="course-meta">${isMain ? 'Main admin' : adminRoleLabel(r.admin_role)}</div>
        </div>
      </div>
...
async function loadAccessList(){
  if (!sb) return;
  try {
    const { data, error } = await sb.from('user_roles').select('email,role,admin_role').order('created_at', { ascending: true });
    if (error) throw error;
...
function bindAdminEvents(){
  ...
  if (addAccessForm){
    addAccessForm.addEventListener('submit', async e => {
      ...
      const adminRole = roleSelect ? roleSelect.value : 'admin';
      if (!email) return;
      try {
        const { error } = await sb.from('user_roles').insert({ email, role: 'admin', admin_role: adminRole, added_by: authSession.user.email });
        if (error) throw error;
        emailInput.value = '';
