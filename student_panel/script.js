...
// Supabase/PostgREST silently caps any .select() at 1000 rows per request
// unless you page through results with .range(). Datasheets can run into
// the thousands of rows (bulk imports), so every unbounded select() that
// could plausibly exceed 1000 rows must be paged through this helper —
// otherwise the extra rows are just dropped, with no error, no warning.
...
async function syncSemestersToSupabase(){
  ...
  try {
    const semesterRows = state.semesters.map(s => ({ id: s.id, name: s.name, is_current: !!s.isCurrent, is_upcoming: !!s.isUpcoming }));
    ...
    const existingSem = await fetchAllRows((from, to) =>
      sb.from('semesters').select('id').range(from, to));
    ...
    for (const sem of state.semesters){
      ...
      const existingCourses = await fetchAllRows((from, to) =>
        sb.from('courses').select('id').eq('semester_id', sem.id).range(from, to));
...
async function refreshSemestersFromSupabase(){
  if (!sb) return false;
  try {
    const sems = await fetchAllRows((from, to) =>
      sb.from('semesters').select('id,name,is_current,is_upcoming').order('created_at', { ascending: true }).range(from, to));
    const courses = await fetchAllRows((from, to) =>
      sb.from('courses').select('*').range(from, to));
    ...
    const semesters = (sems || []).map(s => ({ id: s.id, name: s.name, isCurrent: !!s.is_current, isUpcoming: !!s.is_upcoming, courses: bySemester[s.id] || [] }));
...
async function onAuthorized(){
  ...
  await loadProfile();
  await loadMyAccessTypes();
}

// Pulls the signed-in admin's own access_types array from user_roles, for
// nav-tab gating (hasAdminAccess) and the profile-menu "Role" line. Only
// meaningful for appRole === 'admin' — the hardcoded main admin's role is
// always "Main Admin" and students don't have a user_roles row at all. A
// non-main admin can read only *their own* row here — see the
// user_roles_select_self policy in
// supabase/migrations/002_admin_roles_and_comments.sql.
async function loadMyAccessTypes(){
  if (!sb || !authSession || appRole !== 'admin'){ state.myAccessTypes = []; return; }
  try {
    const email = (authSession.user.email || '').toLowerCase();
    const { data, error } = await sb.from('user_roles').select('access_types').eq('email', email).maybeSingle();
    if (error) throw error;
    state.myAccessTypes = (data && Array.isArray(data.access_types)) ? data.access_types : [];
  } catch (e){
    console.error('Could not load your access types.', e);
    state.myAccessTypes = [];
  }
}

// Role shown in the profile-menu dropdown — reflects the admin's granted
// access type(s) (or "Main Admin") or plain "Student".
function getRoleLabel(){
  if (appRole === 'main_admin') return 'Main Admin';
  if (appRole === 'admin') return accessTypeLabels(state.myAccessTypes);
  if (appRole === 'student') return 'Student';
  return '';
}

// Logs one login for the signed-in account via a SECURITY DEFINER RPC (see
// record_login() in supabase/schema.sql), which upserts a row in `login_log`
// keyed by email and bumps its login_count. Guarded by loginRecorded so a
// token refresh or tab focus mid-session doesn't double-count a visit — only
// the first successful authorization per page load logs a login. Powers the
// admin Dashboard's "total student logins" stat and the Student directory.
let loginRecorded = false;
async function recordLoginRemote(){
  if (!sb) return;
  try {
    const { error } = await sb.rpc('record_login');
    if (error) throw error;
  } catch (e){
    console.error('Could not record this login.', e);
  }
}

// Pulls the signed-in account's own row from `profiles` (name they've set on
// the student panel's Profile page). Anyone can read/write only their own row
// — see the profiles_select/profiles_upsert/profiles_update policies.
async function loadProfile(){
  if (!sb || !authSession) return;
  try {
    const email = (authSession.user.email || '').toLowerCase();
    const { data, error } = await sb.from('profiles').select('name').eq('email', email).maybeSingle();
    if (error) throw error;
    state.profile = { name: (data && data.name) || '' };
  } catch (e){
...
// Sets one semester as "ongoing" (and un-sets any other), for the admin
// Dashboard's "ongoing semester" / "total ongoing courses" stats.
function setOngoingSemester(semId){
  state.semesters.forEach(s => { s.isCurrent = (s.id === semId); });
  saveSemesters();
  renderMain();
}

function getOngoingSemester(){
  return state.semesters.find(s => s.isCurrent) || null;
}

// Sets one semester as "upcoming" (and un-sets any other) — independent of
// which semester (if any) is marked "ongoing" — for the admin Dashboard's
// "upcoming semester" stat.
function setUpcomingSemester(semId){
  state.semesters.forEach(s => { s.isUpcoming = (s.id === semId); });
  saveSemesters();
  renderMain();
}

function getUpcomingSemester(){
  return state.semesters.find(s => s.isUpcoming) || null;
}

// Admin Dashboard: total student logins, summed from login_log (readable by
// admin/main_admin only — see the login_log_select policy).
async function loadDashboardStats(){
  if (!sb) return;
  try {
    // count-only request — we just need how many distinct students have
    // ever signed in, not how many times each of them has logged in.
    const { count, error } = await sb.from('login_log').select('email', { count: 'exact', head: true }).eq('role', 'student');
    if (error) throw error;
    state.dashboardStats.totalStudents = count || 0;
  } catch (e){
    console.error('Could not load dashboard stats.', e);
  }
  state.dashboardStats.loaded = true;
  renderMain();
}

// Admin "Student" page: every student who has ever logged in (from
// login_log), with their profile name if they've set one.
async function loadStudentsList(){
  if (!sb) return;
  try {
    const { data: logins, error } = await sb
      .from('login_log').select('email,login_count,last_login')
      .eq('role', 'student').order('last_login', { ascending: false });
    if (error) throw error;
    const { data: profiles, error: pErr } = await sb.from('profiles').select('email,name');
    if (pErr) throw pErr;
    const nameByEmail = {};
    (profiles || []).forEach(p => { nameByEmail[p.email] = p.name; });
    state.studentsList = (logins || []).map(r => ({
      email: r.email,
    ...
    console.error('Could not load the student list.', e);
    state.studentsList = state.studentsList || [];
  }
  state.studentsListLoaded = true;
  renderMain();
}
...
function signOut(){
  ...
  sb.auth.signOut().then(() => {
    // Land back on the clean home URL (https://uni-co-plan.vercel.app/)
    // instead of staying on this panel's own login screen.
...
const state = {
  ...
  adminPage: 'dashboard',   // 'dashboard' | 'semesters' | 'datasheet' | 'students' | 'comments' | 'access'
  semesters: [],
  adminSemesterId: null,
  studentSemesterId: null,
  studentPage: 'dashboard', // 'dashboard' | 'profile' | 'plan' | 'timetable' | 'combinations' | 'comments'
  editingCourseId: null,
  selections: {},   // semesterId -> [courseId, ...]  (max 5 per semester)
  combinations: {}, // semesterId -> [{ id, name, courseIds, createdAt }, ...]
  search: '',
  adminSearch: '',
  studentSort: 'az', // 'az' | 'za' — sort order for the student "Available Courses" list
  accessList: [], // rows from user_roles — loaded on demand when the main admin opens "Manage access"
  accessListLoaded: false,
  dashboardStats: { totalStudents: 0, loaded: false }, // admin "Dashboard" page
  studentsList: [],       // rows from login_log (+ name from profiles) — admin "Student" page
  studentsListLoaded: false,
  studentsSearch: '',
  profile: { name: '' },  // the signed-in account's own row from `profiles` (student panel "Profile" page)
  profileLoaded: false,
  myAccessTypes: [],      // this admin's own granted access_types (from user_roles), for nav gating + the profile-menu "Role" line
  previewAsStudent: false, // admin panel only: true while showing the student view inside "Preview as Student"
  myComments: [],          // this student's own rows from `student_comments` (student "Comments" + "Dashboard" pages)
  myCommentsLoaded: false,
  allComments: [],         // every row from `student_comments` (admin "Comments" page)
  allCommentsLoaded: false,
  publicStudentCount: 0,     // total distinct students ever signed in — shown on the pre-login screen (both index.html and here) to encourage sign-ups
  publicStudentCountLoaded: false
};
const MAX_PLAN_COURSES = 5;
let importPreviewData = [];

// Admin access types shown as multi-select checkboxes in "Manage access"
// and in a signed-in admin's own profile-menu "Role" line. Extendable —
// add more { value, label } entries here as new admin access types are
// needed. `value` is what's stored in user_roles.access_types (a Postgres
// text[]) — an admin can be granted any combination of these; `main_admin`
// is treated client-side as "all of the below", though it does NOT grant
// the real, database-level main-admin permissions (those stay fixed to the
// one hardcoded account — see current_app_role() in supabase/schema.sql).
const ADMIN_ACCESS_TYPES = [
  { value: 'main_admin', label: 'Main Admin' },
  { value: 'semester_adjust', label: 'Semester Adjust' },
  { value: 'datasheet_edit', label: 'Course datasheet edit' },
  { value: 'students', label: 'Students' },
  { value: 'comments', label: 'Student comments' }
];
function accessTypeLabels(values){
  const arr = Array.isArray(values) ? values : [];
  if (arr.includes('main_admin')) return 'Main Admin';
  if (arr.length === 0) return 'Admin';
  return arr.map(v => {
    const found = ADMIN_ACCESS_TYPES.find(t => t.value === v);
    return found ? found.label : v;
  }).join(', ');
}
// Whether the signed-in admin can see/use a given admin-panel section.
// main_admin always can; a regular admin needs that specific access type
// (or 'main_admin', granted to them as a broad "everything" type) in their
// own access_types. This is client-side page gating only — it decides
// which nav tabs render, not what Supabase itself will allow; the
// underlying RLS policies are unchanged (see supabase/schema.sql).
function hasAdminAccess(type){
  if (appRole === 'main_admin') return true;
  if (appRole !== 'admin') return false;
  const types = state.myAccessTypes || [];
  return types.includes('main_admin') || types.includes(type);
}
...
// Top-right profile menu (both panels): avatar → dropdown with Gmail ID,
// Name, Role (admin sub-type or "Student"), and Sign out.
//
// The dropdown's markup is rendered into #profileMenuLayer — a sibling of
// #header in the HTML, not a descendant of it — and shown with
// position:fixed. That's deliberate: #header has overflow:hidden (for its
// own decorative glow), which would otherwise silently clip the dropdown
// to invisible the moment it tried to open below the header.
function renderHeader(){
  const header = document.getElementById('header');
  const menuLayer = document.getElementById('profileMenuLayer');
  const avatarUrl = authSession && authSession.user.user_metadata ? authSession.user.user_metadata.avatar_url : null;
  const email = authSession ? (authSession.user.email || '') : '';
  const initials = email ? email.charAt(0).toUpperCase() : '?';
  const displayName = (state.profile && state.profile.name) ? state.profile.name : '';
  const roleLabel = getRoleLabel();

  // Logo stays perfectly centered because, with .header-user taken out of
  // normal flow (position:absolute, pinned to the top-right corner — see
  // style.css), .header-brand is #header's only flex item, so
  // justify-content:center centers it regardless of whether anyone's signed in.
  const userBlock = authSession ? `
    <div class="header-user" id="profileMenuWrap">
      <button type="button" class="profile-trigger" id="profileMenuBtn" aria-haspopup="true" aria-expanded="false">
        ${avatarUrl ? `<img class="header-avatar" src="${escapeHtml(avatarUrl)}" alt="">` : `<span class="header-avatar header-avatar-fallback">${escapeHtml(initials)}</span>`}
        <span class="header-username">${escapeHtml(email)}</span>
  ...
  else header.classList.remove('header-has-logout');

  if (menuLayer){
    menuLayer.innerHTML = authSession ? `
      <div class="profile-dropdown" id="profileDropdown" role="menu">
        <div class="profile-dropdown-row">
          <span class="profile-dropdown-label">Name</span>
          <span class="profile-dropdown-value">${escapeHtml(displayName || '—')}</span>
        </div>
        <div class="profile-dropdown-row">
          <span class="profile-dropdown-label">Gmail</span>
          <span class="profile-dropdown-value mono">${escapeHtml(email)}</span>
        </div>
        <div class="profile-dropdown-row">
          <span class="profile-dropdown-label">Role</span>
          <span class="profile-dropdown-value">${escapeHtml(roleLabel)}</span>
        </div>
        <button type="button" class="profile-dropdown-signout" id="signOutBtn">Sign out</button>
      </div>
    ` : '';
  }
...
// Closes the profile dropdown (used by the outside-click/scroll/resize
// listeners below, attached once at module load rather than inside
// renderHeader, so repeated header re-renders don't pile up duplicate
...
// Places the fixed-position dropdown directly under the avatar trigger,
// clamped so it never runs off the right or left edge of the viewport.
...
if (typeof document.addEventListener === 'function'){
  document.addEventListener('click', e => {
    ...
    if ((menuBtn && menuBtn.contains(e.target)) || dropdown.contains(e.target)) return;
    closeProfileMenu();
  });
}
...
// Shown when nobody is signed in yet — neither panel shows any app content
// until Google sign-in succeeds. On the student panel, this also shows the
// running "Total students signed in" count (student_login_count() RPC,
// callable while signed out) as social proof to encourage new students to
// sign in and use the site.
function renderLoginScreen(){
  const main = document.getElementById('mainView');
  const panelLabel = ROLE === 'admin' ? 'admin panel' : 'student panel';
  const statLine = ROLE === 'student'
    ? (state.publicStudentCountLoaded
        ? `<p class="login-stat">🎉 ${state.publicStudentCount} student${state.publicStudentCount === 1 ? '' : 's'} already planning their semester on UniCoPlan.</p>`
        : `<p class="login-stat login-stat-loading">&nbsp;</p>`)
    : '';
  main.innerHTML = `
  ...
  if (ROLE === 'student' && !state.publicStudentCountLoaded) loadPublicStudentCount();
}

// Total distinct students who have ever signed in — safe to call whether or
// not anyone's currently signed in (the student_login_count() RPC is
// intentionally public: it returns only a count, never any student's email
// or login history, which stay admin-only via login_log's own RLS policy).
async function loadPublicStudentCount(){
  if (!sb) return;
  try {
    const { data, error } = await sb.rpc('student_login_count');
    if (error) throw error;
    state.publicStudentCount = data || 0;
  } catch (e){
    console.error('Could not load the total signed-in student count.', e);
  }
  state.publicStudentCountLoaded = true;
  renderApp();
}
  ...
  main.innerHTML = `
    <div class="login-wrap">
      <div class="login-card form-card">
        ...
        ${showStudentLink ? `<p class="login-sub"><a href="/student">Go to the student panel instead →</a></p>` : ''}
...
function renderMain(){
  const main = document.getElementById('mainView');
  // Admin "Preview as Student": renders the exact same student template/
  // events used by the real student panel, just with a banner on top and
  // under the admin's own signed-in session — so it always reflects
  // whatever the admin just published.
  if (ROLE === 'admin' && state.previewAsStudent){
    main.innerHTML = previewBannerHTML() + studentTemplate();
    bindPreviewBannerEvents();
    bindStudentEvents();
    renderAvailableList();
    renderTimetable();
    renderSelectedList();
...
// Banner shown at the top of "Preview as Student" mode (admin panel only).
function previewBannerHTML(){
  return `
    <div class="preview-banner">
      <span>👁 Previewing as student — this is what students currently see.</span>
      <button type="button" class="btn btn-ghost" id="exitPreviewBtn">Exit preview</button>
    </div>
  `;
}

function bindPreviewBannerEvents(){
  const btn = document.getElementById('exitPreviewBtn');
  if (btn){
    btn.addEventListener('click', () => {
      state.previewAsStudent = false;
      renderMain();
    });
  }
}
...
function semesterTabsHTML(activeId){
  ...
  return `<div class="tabs">${state.semesters.map(s => `
    <div class="tab ${s.id === activeId ? 'active' : ''}" data-tab="${s.id}" role="tab" tabindex="0">
      <button type="button" class="tab-star ${s.isCurrent ? 'tab-star-on' : ''}" data-set-ongoing="${s.id}" title="${s.isCurrent ? 'Ongoing semester' : 'Mark as ongoing semester'}" aria-label="${s.isCurrent ? 'Ongoing semester' : 'Mark as ongoing semester'}">${s.isCurrent ? '★' : '☆'}</button>
      <button type="button" class="tab-upcoming ${s.isUpcoming ? 'tab-upcoming-on' : ''}" data-set-upcoming="${s.id}" title="${s.isUpcoming ? 'Upcoming semester' : 'Mark as upcoming semester'}" aria-label="${s.isUpcoming ? 'Upcoming semester' : 'Mark as upcoming semester'}">⏭</button>
...
function adminPageNavHTML(){
  ...
  if (hasAdminAccess('semester_adjust')) items.push({ key:'semesters', label:'Semesters' });
  if (hasAdminAccess('datasheet_edit')) items.push({ key:'datasheet', label:'Course datasheet edit' });
  if (hasAdminAccess('students')) items.push({ key:'students', label:'Student' });
  if (hasAdminAccess('comments')) items.push({ key:'comments', label:'Comments' });
  ...
  return `<div class="page-nav">
    ...
    `).join('')}
    <button type="button" class="page-nav-btn page-nav-btn-preview" id="previewStudentBtn">👁 Preview as Student</button>
  </div>`;
}

function adminTemplate(){
  // Defensive fallback: if state.adminPage points at a section this admin
  // no longer (or never did) have the access type for — e.g. access was
  // just revoked, or access_types hadn't finished loading yet on first
  // render — send them back to Dashboard rather than rendering a page
  // whose own nav tab isn't even shown.
  if (state.adminPage === 'semesters' && !hasAdminAccess('semester_adjust')) state.adminPage = 'dashboard';
  if (state.adminPage === 'datasheet' && !hasAdminAccess('datasheet_edit')) state.adminPage = 'dashboard';
  if (state.adminPage === 'students' && !hasAdminAccess('students')) state.adminPage = 'dashboard';
  if (state.adminPage === 'comments' && !hasAdminAccess('comments')) state.adminPage = 'dashboard';
  if (state.adminPage === 'access' && appRole !== 'main_admin') state.adminPage = 'dashboard';

  const nav = adminPageNavHTML();

  if (state.adminPage === 'dashboard') return nav + adminDashboardTemplate();
  if (state.adminPage === 'datasheet') return nav + adminDatasheetEditTemplate();
  if (state.adminPage === 'students') return nav + adminStudentsTemplate();
  if (state.adminPage === 'comments') return nav + adminCommentsTemplate();
      ...
      c.meetings.forEach(m => {
        ...
        blocksHtml += `
          <div class="course-block" style="left:${left}px;width:${width}px;background:${COLORS[colorIdx].bg};border-left-color:${COLORS[colorIdx].bd}">
            ...
            ${m.room ? `<div class="block-meta block-room">Room ${escapeHtml(m.room)}</div>` : ''}
          </div>
        `;
      });
    });
...
// Top-level admin landing page: a quick read on how the platform is doing —
// how many times students have logged in in total, which semester is
// currently marked "ongoing" (starred on the Semesters tab), and how many
// courses that semester has published.
function adminDashboardTemplate(){
  const ongoing = getOngoingSemester();
  const upcoming = getUpcomingSemester();
  const stats = state.dashboardStats;
  return `
    <section class="stat-grid">
      <div class="stat-card">
        <span class="stat-label">Total students signed in</span>
        <span class="stat-value">${stats.loaded ? stats.totalStudents : '…'}</span>
        <span class="stat-sub">${stats.loaded ? `Distinct student account${stats.totalStudents === 1 ? '' : 's'} that have signed in` : 'Loading…'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Ongoing semester</span>
        <span class="stat-value">${ongoing ? escapeHtml(ongoing.name) : '—'}</span>
        <span class="stat-sub">${ongoing ? `${ongoing.courses.length} course${ongoing.courses.length === 1 ? '' : 's'} published` : 'Mark ★ on the Semesters tab to set one'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Upcoming semester</span>
        <span class="stat-value">${upcoming ? escapeHtml(upcoming.name) : '—'}</span>
        <span class="stat-sub">${upcoming ? `${upcoming.courses.length} course${upcoming.courses.length === 1 ? '' : 's'} published` : 'Mark ⏭ on the Semesters tab to set one'}</span>
      </div>
    </section>
  `;
}

// Directory of every student who has ever signed in — their email (always),
// and their name if they've set one on the student panel's Profile page.
function adminStudentsTemplate(){
  const rows = state.studentsList || [];
  return `
    <section class="table-card">
      <div class="table-card-head">
        <h3>Students <span class="muted">— ${rows.length} signed in</span></h3>
        <div class="search-wrap admin-search-wrap">
          <input type="search" id="studentsSearch" placeholder="Search email or name" value="${escapeHtml(state.studentsSearch)}">
        </div>
      </div>
...
function renderStudentsTable(){
  ...
  const rows = state.studentsList || [];
  let filtered = rows;
  if (state.studentsSearch && state.studentsSearch.trim()){
    const q = state.studentsSearch.trim().toLowerCase();
    filtered = rows.filter(r => r.email.toLowerCase().includes(q) || (r.name || '').toLowerCase().includes(q));
  }
  if (!state.studentsListLoaded){ container.innerHTML = `<p class="empty-note">Loading…</p>`; return; }
  if (rows.length === 0){ container.innerHTML = `<p class="empty-note">No students have signed in yet.</p>`; return; }
  if (filtered.length === 0){ container.innerHTML = `<p class="empty-note">No students match your search.</p>`; return; }
  container.innerHTML = `
    <table class="ledger">
      <thead><tr><th>Email</th><th>Name</th><th>Logins</th><th>Last login</th></tr></thead>
      <tbody>
        ${filtered.map(r => `
          <tr>
            <td class="mono">${escapeHtml(r.email)}</td>
            <td>${r.name ? escapeHtml(r.name) : '<span class="muted">Not added</span>'}</td>
            <td>${r.loginCount}</td>
            <td>${r.lastLogin ? new Date(r.lastLogin).toLocaleString() : '—'}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
...
function adminAccessTemplate(){
  ...
  const checkboxes = ADMIN_ACCESS_TYPES.map(t => `
    <label class="access-type-check">
      <input type="checkbox" name="accessTypes" value="${t.value}" ${t.value === 'semester_adjust' ? 'checked' : ''}>
      ${escapeHtml(t.label)}
    </label>
  `).join('');
  return `
    <section class="form-card" style="max-width:640px">
      <h3>Manage access</h3>
      ...
      <form id="addAccessForm">
        <label>Gmail
          <input type="email" id="newAdminEmail" placeholder="name@gmail.com" required>
        </label>
        <fieldset class="access-type-picker">
          <legend>Access type(s) <span class="muted">— select one or more</span></legend>
          ${checkboxes}
        </fieldset>
        <button type="submit" class="btn btn-ghost">+ Grant admin access</button>
      </form>
      <p class="empty-note" style="padding:0 0 4px">"Main Admin" here only labels the account and unlocks every admin tab for them — the account with full, irreversible main-admin access (including this Manage access page itself) is fixed in the database and can't be changed from here.</p>
    ...
    <div class="course-row" data-access-email="${escapeHtml(r.email)}">
      <div class="course-row-main">
        <div>
          ...
          <div class="course-meta">${isMain ? 'Main admin' : escapeHtml(accessTypeLabels(r.access_types))}</div>
        </div>
      </div>
...
async function loadAccessList(){
  if (!sb) return;
  try {
    const { data, error } = await sb.from('user_roles').select('email,role,access_types').order('created_at', { ascending: true });
    if (error) throw error;
...
// Admin "Comments" page — every message students have sent via the student
// panel's own Comments page (step 4), newest first.
function adminCommentsTemplate(){
  const rows = state.allComments || [];
  return `
    <section class="table-card">
      <div class="table-card-head">
        <h3>Student comments <span class="muted">— ${rows.length} message${rows.length === 1 ? '' : 's'}</span></h3>
      </div>
...
function renderCommentsTable(){
  ...
  if (!container) return;
  const rows = state.allComments || [];
  if (!state.allCommentsLoaded){ container.innerHTML = `<p class="empty-note">Loading…</p>`; return; }
  if (rows.length === 0){ container.innerHTML = `<p class="empty-note">No comments or update requests yet.</p>`; return; }
  container.innerHTML = `
    <table class="ledger">
      <thead><tr><th>Student</th><th>Message</th><th>Sent</th></tr></thead>
      <tbody>
        ${rows.map(r => `
          <tr>
            <td>
              <div>${r.name ? escapeHtml(r.name) : '<span class="muted">Not added</span>'}</div>
              <div class="course-meta mono">${escapeHtml(r.email)}</div>
            </td>
            <td>${escapeHtml(r.message)}</td>
            <td class="mono">${r.createdAt ? new Date(r.createdAt).toLocaleString() : '—'}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
...
async function loadAllComments(){
  if (!sb) return;
  try {
    const { data, error } = await sb.from('student_comments').select('id,email,name,message,created_at').order('created_at', { ascending: false });
    if (error) throw error;
    state.allComments = (data || []).map(r => ({ id: r.id, email: r.email, name: r.name, message: r.message, createdAt: r.created_at }));
  } catch (e){
    console.error('Could not load student comments.', e);
...
function bindAdminEvents(){
  ...
  // Fetches the Dashboard stats / Student directory / Comments the first
  // time their page is shown — whether that's from clicking its tab, or
  // from landing on 'dashboard' straight after sign-in, since it's the
  // default admin page. The loaded flags keep this from re-fetching on
  // every re-render.
  if (state.adminPage === 'dashboard' && !state.dashboardStats.loaded) loadDashboardStats();
  if (state.adminPage === 'students' && !state.studentsListLoaded) loadStudentsList();
  if (state.adminPage === 'comments' && !state.allCommentsLoaded) loadAllComments();

  const previewBtn = document.getElementById('previewStudentBtn');
  if (previewBtn){
    previewBtn.addEventListener('click', () => {
      state.previewAsStudent = true;
      renderMain();
    });
  }

  document.querySelectorAll('[data-set-ongoing]').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
      setOngoingSemester(btn.dataset.setOngoing);
    });
  });
  document.querySelectorAll('[data-set-upcoming]').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
      setUpcomingSemester(btn.dataset.setUpcoming);
    });
  });
  ...
  if (studentsSearch){
    studentsSearch.addEventListener('input', () => {
      state.studentsSearch = studentsSearch.value;
      renderStudentsTable();
    });
  }
  ...
  if (addAccessForm){
    addAccessForm.addEventListener('submit', async e => {
      ...
      const accessTypes = Array.from(document.querySelectorAll('input[name="accessTypes"]:checked')).map(cb => cb.value);
      if (!email) return;
      try {
        const { error } = await sb.from('user_roles').insert({ email, role: 'admin', access_types: accessTypes, added_by: authSession.user.email });
        if (error) throw error;
        emailInput.value = '';
  ...
  if (newSemForm){
    newSemForm.addEventListener('submit', e => {
      ...
      if (!name) return;
      const sem = { id: uid('sem_'), name, courses: [], isCurrent: false, isUpcoming: false };
...
function studentPageNavHTML(){
  const items = [
    ...
    { key: 'plan', label: 'Available Courses' },
...
// A compact "select a semester" dropdown reused at the top of the Class
// Schedule and Combinations pages (the Available Courses page has its own,
// fuller toolbar below). Changing it updates the same shared
// state.studentSemesterId as the main one, so all three pages stay in sync.
function studentSemesterPickerHTML(id){
  return `
...
function studentProfileTemplate(){
  const email = authSession ? authSession.user.email : '';
  return `
    <section class="form-card" style="max-width:480px">
      <h3>Profile</h3>
      <div id="profileFlash" class="flash"></div>
      ${!state.profileLoaded ? `<p class="empty-note">Loading…</p>` : `
        <form id="profileForm">
          <label>Email
            <input type="email" value="${escapeHtml(email)}" disabled>
          </label>
          <label>Full name
            <input type="text" id="profileName" name="name" placeholder="Your name" value="${escapeHtml(state.profile.name || '')}" required>
          </label>
          <button type="submit" class="btn btn-primary">Save profile</button>
        </form>
        <p class="empty-note" style="padding-bottom:0">Your name (once saved) is what shows up next to your email on the admin's Student page.</p>
      `}
    </section>
  `;
}
...
async function saveProfile(form){
  ...
  if (!sb || !authSession){ if (flash) showFlash(flash, 'error', 'You need to be signed in to save a profile.'); return; }
  const fd = new FormData(form);
  const name = (fd.get('name') || '').trim();
  const email = (authSession.user.email || '').toLowerCase();
  if (!name){ if (flash) showFlash(flash, 'error', 'Enter your name first.'); return; }
  try {
    const { error } = await sb.from('profiles').upsert({ email, name });
    if (error) throw error;
    state.profile = { name };
    if (flash) showFlash(flash, 'success', 'Profile saved.');
  } catch (e){
    console.error('Could not save profile.', e);
    if (flash) showFlash(flash, 'error', 'Could not save your profile — try again.');
  }
}

// Student's own Dashboard — same stat-grid layout as the admin Dashboard
// (step 5), adapted to what a student can see: the published ongoing/
// upcoming semester (readable by any signed-in student, same as admin sees
// it) plus this student's own plan and message counts, which are the
// "personal" stats available at student scope.
function studentDashboardTemplate(){
  const ongoing = getOngoingSemester();
  const upcoming = getUpcomingSemester();
  const planSem = getSemester(state.studentSemesterId) || ongoing || state.semesters[0] || null;
  const planCount = planSem ? (state.selections[planSem.id] || []).length : 0;
  return `
    <section class="stat-grid">
      <div class="stat-card">
        <span class="stat-label">Ongoing semester</span>
        <span class="stat-value">${ongoing ? escapeHtml(ongoing.name) : '—'}</span>
        <span class="stat-sub">${ongoing ? `${ongoing.courses.length} course${ongoing.courses.length === 1 ? '' : 's'} published` : 'Not set yet by the admin'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Upcoming semester</span>
        <span class="stat-value">${upcoming ? escapeHtml(upcoming.name) : '—'}</span>
        <span class="stat-sub">${upcoming ? `${upcoming.courses.length} course${upcoming.courses.length === 1 ? '' : 's'} published` : 'Not set yet by the admin'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Your plan</span>
        <span class="stat-value">${planSem ? `${planCount}/${MAX_PLAN_COURSES}` : '—'}</span>
        <span class="stat-sub">${planSem ? `Courses selected for ${escapeHtml(planSem.name)}` : 'Select a semester on Available Courses'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Your messages</span>
        <span class="stat-value">${state.myCommentsLoaded ? state.myComments.length : '…'}</span>
        <span class="stat-sub">Comments/update requests you've sent to the admin</span>
      </div>
    </section>
  `;
}

// Student "Comments" page (step 4) — a message box that writes into the
// shared `student_comments` table, plus a running list of this student's
// own past messages.
function studentCommentsTemplate(){
  return `
    <section class="form-card" style="max-width:560px">
      <h3>Comments &amp; update requests</h3>
      <p class="empty-note" style="padding-top:0">Tell the admin about anything related to your course or semester info — corrections, clashes, or update requests.</p>
      <div id="commentFlash" class="flash"></div>
      <form id="commentForm">
        <label>Message
          <textarea id="commentMessage" name="message" rows="4" required placeholder="e.g. CSE303-2's room number on the datasheet looks wrong…"></textarea>
        </label>
        <button type="submit" class="btn btn-primary">Send</button>
      </form>
    </section>
    <section class="table-card" style="margin-top:22px; max-width:560px">
      <h3>Your messages</h3>
...
function renderMyComments(){
  ...
  if (!state.myCommentsLoaded){ container.innerHTML = `<p class="empty-note">Loading…</p>`; return; }
  if (rows.length === 0){ container.innerHTML = `<p class="empty-note">You haven't sent any messages yet.</p>`; return; }
  container.innerHTML = rows.map(r => `
    <div class="course-row">
      <div class="course-title" style="font-weight:400">${escapeHtml(r.message)}</div>
      <div class="course-meta mono">${r.createdAt ? new Date(r.createdAt).toLocaleString() : ''}</div>
    </div>
  `).join('');
}

// This student's own rows from `student_comments` — powers both the
// Comments page's history list and the Dashboard's "Your messages" stat.
async function loadMyComments(){
  if (!sb || !authSession) return;
  try {
    const email = (authSession.user.email || '').toLowerCase();
    const { data, error } = await sb.from('student_comments').select('id,message,created_at').eq('email', email).order('created_at', { ascending: false });
    if (error) throw error;
    state.myComments = (data || []).map(r => ({ id: r.id, message: r.message, createdAt: r.created_at }));
  } catch (e){
    console.error('Could not load your messages.', e);
    state.myComments = state.myComments || [];
  }
  state.myCommentsLoaded = true;
  renderMain();
}
...
function studentTemplate(){
  ...
  if (state.studentPage === 'dashboard') return nav + studentDashboardTemplate();
  if (state.studentPage === 'comments') return nav + studentCommentsTemplate();
  if (state.studentPage === 'profile') return nav + studentProfileTemplate();
  ...
  if (state.studentPage === 'timetable'){
    ...
    return nav + `
      <section class="student-toolbar" style="margin-bottom:18px">
        ${studentSemesterPickerHTML('timetableSemesterSelect')}
      </section>
  ...
  return nav + `
    <section class="student-toolbar">
      ${studentSemesterPickerHTML('semesterSelect')}
      <div class="search-wrap">
        <label for="courseSearch">Search</label>
        <input type="search" id="courseSearch" value="${escapeHtml(state.search)}">
      </div>
      <div class="semester-select-wrap">
        <label for="courseSort">Sort</label>
        <select id="courseSort">
          <option value="az" ${state.studentSort === 'az' ? 'selected' : ''}>Course A–Z</option>
          <option value="za" ${state.studentSort === 'za' ? 'selected' : ''}>Course Z–A</option>
        </select>
      </div>
    </section>
    ${state.semesters.length === 0 ? `
...
function combinationsPageTemplate(){
  ...
  return `
    <section class="student-toolbar" style="margin-bottom:18px">
      ${studentSemesterPickerHTML('comboSemesterSelect')}
    </section>
    <section class="schedule-card">
      <div class="timetable-page-head">
        <h3>Saved combinations ${sem ? `<span class="muted">— ${escapeHtml(sem.name)}</span>` : ''}</h3>
      </div>
      ${!sem ? `<p class="empty-note">Select a semester above first.</p>` :
        combos.length === 0 ? `<p class="empty-note">No saved combinations yet. Build a plan on the Available Courses tab, then use "Save as combination" from the Class Schedule tab.</p>` :
...
function bindStudentEvents(){
  ...
  // Fetches this student's own messages the first time the Dashboard (for
  // its "Your messages" stat) or Comments page is shown. The loaded flag
  // keeps this from re-fetching on every re-render.
  if ((state.studentPage === 'dashboard' || state.studentPage === 'comments') && !state.myCommentsLoaded) loadMyComments();
  ...
  if (commentForm){
    commentForm.addEventListener('submit', async e => {
      ...
      if (!sb || !authSession){ if (flash) showFlash(flash, 'error', 'You need to be signed in to send a message.'); return; }
      ...
      try {
        const { error } = await sb.from('student_comments').insert({ id: uid('cmt_'), email, name, message });
        if (error) throw error;
        textarea.value = '';
        if (flash) showFlash(flash, 'success', "Sent — the admin can see this on their Comments page.");
        state.myCommentsLoaded = false;
        await loadMyComments();
      } catch (err){
        console.error('Could not send message.', err);
        if (flash) showFlash(flash, 'error', 'Could not send your message — try again.');
      }
    });
  }
  ...
  // The same semester picker also appears (with different IDs) atop the
  // Class Schedule and Combinations pages — all three write to the one
  // shared state.studentSemesterId.
  const timetableSel = document.getElementById('timetableSemesterSelect');
  if (timetableSel){
    timetableSel.addEventListener('change', () => {
      state.studentSemesterId = timetableSel.value;
      renderMain();
    });
  }
  const comboSel = document.getElementById('comboSemesterSelect');
  if (comboSel){
    comboSel.addEventListener('change', () => {
      state.studentSemesterId = comboSel.value;
      renderMain();
    });
  }
...
function renderAvailableList(){
  ...
  // sem.courses is already kept sorted A–Z by course code/section (see
  // sortCourses) — Z–A is just that same order reversed.
  courses = state.studentSort === 'za' ? courses.slice().reverse() : courses;
  if (courses.length === 0){
    container.innerHTML = `<p class="empty-note">No courses match your search.</p>`;
    return;
  }
