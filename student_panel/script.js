...
async function loadDashboardStats(){
  if (!sb) return;
  try {
    const { data, error } = await sb.from('login_log').select('login_count').eq('role', 'student');
    if (error) throw error;
    const rows = data || [];
    state.dashboardStats.totalStudentLogins = rows.reduce((sum, r) => sum + (r.login_count || 0), 0);
    state.dashboardStats.totalStudents = rows.length;
  } catch (e){
...
const state = {
  ...
  dashboardStats: { totalStudentLogins: 0, totalStudents: 0, loaded: false }, // admin "Dashboard" page
...
function adminDashboardTemplate(){
  ...
  return `
    <section class="stat-grid">
      <div class="stat-card">
        <span class="stat-label">Total student logins</span>
        <span class="stat-value">${stats.loaded ? stats.totalStudentLogins : '…'}</span>
        <span class="stat-sub">${stats.loaded ? `${stats.totalStudents} student${stats.totalStudents === 1 ? '' : 's'} have signed in` : 'Loading…'}</span>
      </div>
      <div class="stat-card">
