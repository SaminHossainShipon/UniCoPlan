...
async function syncSemestersToSupabase(){
  ...
  try {
    const semesterRows = state.semesters.map(s => ({ id: s.id, name: s.name, is_current: !!s.isCurrent }));
...
async function refreshSemestersFromSupabase(){
  ...
  try {
    const sems = await fetchAllRows((from, to) =>
      sb.from('semesters').select('id,name,is_current').order('created_at', { ascending: true }).range(from, to));
    ...
    const semesters = (sems || []).map(s => ({ id: s.id, name: s.name, isCurrent: !!s.is_current, courses: bySemester[s.id] || [] }));
  ...
  return `
    <section class="stat-grid">
      ...
      <div class="stat-card">
        ...
        <span class="stat-sub">${ongoing ? 'Starred on the Semesters tab' : 'Star a semester on the Semesters tab to set one'}</span>
      </div>
      <div class="stat-card">
        <span class="stat-label">Total ongoing courses</span>
        <span class="stat-value">${ongoing ? ongoing.courses.length : '—'}</span>
        <span class="stat-sub">${ongoing ? `Courses published in ${escapeHtml(ongoing.name)}` : 'No ongoing semester set'}</span>
      </div>
    </section>
  `;
}
...
function bindAdminEvents(){
  ...
  if (newSemForm){
    newSemForm.addEventListener('submit', e => {
      ...
      if (!name) return;
      const sem = { id: uid('sem_'), name, courses: [], isCurrent: false };
