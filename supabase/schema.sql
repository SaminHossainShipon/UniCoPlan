...
create table if not exists public.semesters (
  ...
  name        text not null,
  is_current  boolean not null default false, -- the semester starred ★ as "ongoing" (admin Dashboard)
  is_upcoming boolean not null default false, -- the semester marked ⏭ as "upcoming" (admin Dashboard)
  created_at  timestamptz not null default now()
);

-- Existing projects created before these columns existed: no-ops if the
-- columns are already there (the create table above already has them for
-- fresh projects), and adds them in place otherwise.
alter table public.semesters add column if not exists is_current boolean not null default false;
alter table public.semesters add column if not exists is_upcoming boolean not null default false;
