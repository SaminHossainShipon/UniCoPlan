...
`courses` tables, linked by `semester_id`). There's also a `user_roles` table
(email → `admin` / `main_admin`) that backs the access control described
below. Both panels connect using the project's public **anon** key, which is
safe to expose client-side — real access control happens entirely through the
tables' Row Level Security policies and the `current_app_role()` /
`adjust_seat()` database functions, not anything client-side.
